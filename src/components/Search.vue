<template>
  <div class="search">
    <input type="text" v-model="searchData.value" @keyup.enter="btnClick" @keyup.esc="clear" @input="search">
    <div class="total">
      <transition name="slider-left">
        <p v-if="searchData.list.length > 0 && type === 'voice'">{{searchData.index > 0 ? `${searchData.index}/${searchData.list.length}` : `${searchData.list.length}`}}</p>
      </transition>
    </div>
